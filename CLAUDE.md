...
- [ ] `kind: "shared_global_read"` (trust_boundaries) 은 **함수 본문이 top-level VarDecl(non-const, type이 array/struct/union/pointer-to-buffer)을 read하는 경우** 만. 의미는 "이 함수는 다른 함수가 채울 수 있는 byte-level 공유 컨테이너에서 읽음 — attacker-controlled bytes일 가능성이 있는 진입점". 해당 global이 실제로는 단순 카운터/플래그/세션 메타데이터일 수도 있음 — 이건 verifier가 burden-of-proof로 quarantine 처리. 즉 false positive를 substrate에 두는 건 의도된 lossless 행동이지 라벨 force-fit이 아님. `const` global, 스칼라 (int/char/enum) global은 자동 제외 — 검출기 룰이 이미 그렇게 정의되어 있음. 이 enum을 gold에 force-fit하지 말 것 — gold 라벨링은 "이 함수가 실제로 attacker-controlled bytes를 읽는가"라는 의미 판단이지 "shared_global_read" 매칭이 아님.
...
- 핵심 문서: `PLAN.md`, `README.md`, `CLAUDE.md`, `datasets/WORKFLOW.md`, `docs/LLM_CONFIG.md`
- Schemas: `schemas/`
- Datasets: `datasets/<project>-<cve>/{metadata.json, source/, gold/, notes.md}`
- LLM 설정: `.env` (gitignored, local) + `.env.example` (template, committed). Provider/key/model swap은 env var 3개로 끝. 자세한 건 `docs/LLM_CONFIG.md`.
